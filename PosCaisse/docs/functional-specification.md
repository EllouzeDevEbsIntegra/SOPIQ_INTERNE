...
- Numérotation : format imprimé `{POS}-{YYYY}-{SEQ:6}` par défaut, et remise à zéro du compteur réglée à part (continue, journalière, mensuelle, annuelle ; par point de vente et/ou par caisse). Unique en multi-caisses : un réglage dont la portée ne se lit pas sur le ticket est refusé à l'enregistrement. Ce qui s'imprime sur le ticket et s'affiche à la caisse se règle séparément de la référence enregistrée (par défaut : la référence entière).
