...
4. **Numérotation sans doublon multi-caisses.** `document_sequence` est verrouillée par `SELECT … FOR UPDATE` (`@Lock(PESSIMISTIC_WRITE)`) dans la transaction de vente ; `ticket_number` est UNIQUE. Le format imprimé (`{POS}-{YYYY}-{SEQ:6}`) et la **portée du compteur** sont deux réglages distincts : `ticket.resetPeriod` (NONE/DAILY/MONTHLY/YEARLY), `ticket.perPos`, `ticket.perRegister`. La clé de `document_sequence` ne porte que ce qui découpe le compteur (`TICKET:PV01|2026`), plus le préfixe imprimé — on peut donc imprimer l'année sans repartir à 1 le 1er janvier. Un réglage n'est accepté que si tout ce qui découpe le compteur se lit sur le ticket, sans quoi deux portées imprimeraient le même numéro ; et un compteur créé démarre au-dessus du plus grand numéro déjà imprimé sous la même forme, jamais à 1. Ce qui s'**imprime** est un troisième réglage (`ticket.displayPattern`) : la référence entière reste en base pour la comptabilité, le papier peut ne porter que le compteur. L'affichage retenu est **enregistré sur la vente** (`sale_order.ticket_display`), non recalculé — une réimpression doit rendre le ticket que le client a gardé ; la recherche porte sur les deux faces.
5. **L'assistant de commande ne crée rien de neuf.** `AssistantDialog.vue` compose des lots en mémoire puis appelle `cart.addLine()` comme n'importe quelle tuile : même prix de variante, mêmes options, même note séparée par des virgules que la boîte « Remarques » relit. Il ne parle pas au serveur et n'encaisse pas — un deuxième chemin vers le panier, jamais un deuxième panier.
6. **Une session ouverte par caisse.** Index unique partiel `ux_register_session_open ON register_session(register_id) WHERE status='OPEN'` + contrôle applicatif avec message humain (409).
7. **Intégrité financière.** Une vente encaissée n'est jamais supprimée : statuts `PAID → CANCELLED / PARTIALLY_REFUNDED / REFUNDED`, entité `refund`, journal de caisse (`register_journal`) et `audit_log`. Les commandes `HELD` (en attente) sont les seules supprimables (jamais numérotées, jamais payées).
8. **Journal de caisse matérialisé.** Chaque événement (ouverture, vente, paiement, annulation, remboursement, entrée/sortie, clôture) écrit une ligne dans `register_journal` : filtrage simple, aucun UNION complexe à l'exécution.
9. **Impression découplée.** Le rendu produit du **texte monospace** calibré 32/42 colonnes (58/80 mm). Le routage (`PrintService`) crée un `print_job` par destination (ticket client + une par destination de préparation concernée, N copies). V1 : impression navigateur via iframe ; l'état `PENDING/PRINTED` permet un agent ESC/POS ultérieur sans changer le modèle.
10. **Sécurité.** JWT HS256 stateless (`JwtAuthFilter`), BCrypt (force 8 pour rester rapide sur PIN), autorités = permissions du rôle ; `@PreAuthorize` sur les contrôleurs + contrôles métier fins dans les services (remise > seuil, remise max par utilisateur, annulation manager…).
11. **Rapports en SQL natif** (`NamedParameterJdbcTemplate`) : agrégations directement dans PostgreSQL, fuseau `Africa/Tunis` pour les regroupements par jour/heure.
12. **Multi-points de vente** : `company → point_of_sale → register → register_session`, toutes les ventes portent entreprise / point de vente / caisse / session / caissier.
13. **i18n** : dictionnaire `utils/i18n.js` (français) prêt à recevoir arabe/anglais ; aucun libellé métier en dur dans les enums affichées.
