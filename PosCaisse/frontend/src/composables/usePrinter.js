...
export function printJobs(jobs, template) {
  return new Promise(resolve => {
    ...
    const logo = template?.showLogo && template?.logoData ? `<img src="${template.logoData}" style="max-width:60%;max-height:80px;display:block;margin:0 auto 6px">` : ''
    const esc = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;')
    let body = ''
    for (const j of jobs) for (let i = 0; i < Math.max(1, j.copies); i++) body += `<div class="t">${j.destinationCode === 'CLIENT' || j.title === 'Ticket client' ? logo : ''}<pre>${esc(j.content)}</pre></div>`
