...
export function printJobs(jobs, template) {
  return new Promise(resolve => {
    ...
    let body = ''
    for (const j of jobs) for (let i = 0; i < Math.max(1, j.copies); i++) body += `<div class="t">${j.destinationCode === 'CLIENT' || j.title === 'Ticket client' ? logo : ''}${receiptHtml(j.content)}</div>`
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Ticket</title><style>
      ...
      .logo { max-width: 60%; max-height: 80px; display: block; margin: 0 auto 4px; filter: grayscale(1) contrast(1.4); }
