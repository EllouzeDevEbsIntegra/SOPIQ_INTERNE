...
onMounted(load)
const filtered = computed(() => rows.value.filter(p => (!catFilter.value || p.categoryId === Number(catFilter.value)) && (!q.value || (p.name + ' ' + p.code + ' ' + (p.reference || '')).toLowerCase().includes(q.value.toLowerCase()))))
...
<template>
  <div class="toolbar"><button class="btn primary" @click="create">+ Nouveau produit / menu</button><input class="input" v-model="q" placeholder="Rechercher…" /><select class="input" v-model="catFilter"><option value="">Toutes catégories</option><option v-for="c in cats" :key="c.id" :value="c.id">{{ c.name }}</option></select><span class="muted small">{{ filtered.length }} produit(s)</span></div>
  <div class="table-wrap"><table class="table">
    <thead><tr><th>Code</th><th>Produit</th><th>Catégorie</th><th class="right">Prix</th><th>Type</th><th>Options</th><th>Impression</th><th>Favori</th><th>Dispo</th><th>Actif</th><th></th></tr></thead>
