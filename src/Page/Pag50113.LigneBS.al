...
{
    AutoSplitKey = true;
    Caption = 'Panier Bon Sortie (Tous)';
    Editable = true;
    LinksAllowed = false;
    PageType = List;
    UsageCategory = Lists;
    ...
    SourceTableView = sorting("No.") where(BS = const(true), "Quantity Invoiced" = filter(= 0), "Qty BS To Invoice" = filter(> 0), Masque = filter(false));
