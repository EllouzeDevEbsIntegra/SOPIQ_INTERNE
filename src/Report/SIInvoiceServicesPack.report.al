report 50224 "Invoice Services_Pack"
{

    RDLCLayout = './src/report/RDLC/InvoiceServices_Pack.rdl';
        ...
        dataitem("Sales Invoice Header"; 112)
        {
            ...
            dataitem("Sales Invoice Line"; 113)
            {
                ...
                begin
                    ...
                    //HATEM
                    TotPackage := "Sales Invoice Line".Amount;
                    Gdescription := '';
                    ...
                    if ("Sales Invoice Line"."Package No." <> '') and ("Sales Invoice Line".Type <> "Sales Invoice Line".Type::" ") then begin
                        TotPackage := 0;
                        "Sales Invoice Line"."Unit Price" := 0;
                        "Sales Invoice Line"."VAT %" := 0;
                    end;

                    if ("Sales Invoice Line"."Package No." <> '') and ("Sales Invoice Line".Type = "Sales Invoice Line".Type::" ") and ("Sales Invoice Line"."No." = ServiceMgtSetupEDMS."Begin Pack") then begin
                        TotPackage := 0;
                        SalesInvoiceLine.Reset();
                        SalesInvoiceLine.SetRange("Package No.", "Sales Invoice Line"."Package No.");
                        SalesInvoiceLine.SetRange("Document No.", "Sales Invoice Line"."Document No.");
                        SalesInvoiceLine.SetFilter(Type, '<>%1', "Sales Invoice Line".Type::" ");
                        if SalesInvoiceLine.FindSet() then
                            repeat
                                TotPackage += SalesInvoiceLine.Amount;
                                VatPercent := SalesInvoiceLine."VAT %";
                            until SalesInvoiceLine.Next() = 0;
                        "Sales Invoice Line".Quantity := 1;
                        "Sales Invoice Line"."VAT %" := VatPercent;
                        "Sales Invoice Line"."Unit Price" := TotPackage;
                        "Sales Invoice Line"."Unit of Measure" := 'PCK';
